...
export class CrudCahche extends Map<string, Map<string, StoreDto>>
{
...
const MapsGlobal: Map<string, CrudCahche> = (() => {
	let ret = new Map<string, CrudCahche>();
	ret.set("store", new CrudCahche("store", 3600 * 24));
	ret.set("users", new CrudCahche("users", -1));
	ret.set("actions", new CrudCahche("actions", 3600 * 1000));
	ret.set("memory", new CrudCahche("memory", 0));

	return ret;
})();


export function GetMapSore(table: string): CrudCahche {
